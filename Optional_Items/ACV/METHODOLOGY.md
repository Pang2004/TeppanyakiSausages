...
Only six labelled workbooks are supplied and no development split is prescribed. We make the following choices explicitly rather than treating the small retrospective score as an independent benchmark.

- **No independent train/validation split.** The deployed method is a deterministic physics-informed ranker, and all six disclosed cases informed feature development and retrospective evaluation. We did not reserve an independent subset or refit a supervised model fold-by-fold. Reserving one or two cases would leave very little evidence for either development or evaluation, especially because only one case contains rich pressure telemetry. We therefore report the six-case score as a feasibility result; this rationale does not make it an unbiased estimate of future performance.
- **Workbook as the evaluation unit.** Cars and timestamps in one workbook share ambient conditions and the same labelled event. They are not treated as independent train/test examples. Whole-workbook results receive equal weight so larger recordings do not dominate. Four consecutive portions per workbook test persistence over shorter windows, but remain correlated robustness observations rather than a replacement validation split.
- **Peer-reference assumption.** The disclosed task has one faulty car per case. Comparing each car against the median of usable peers assumes most cars provide a reasonable cooling reference under broadly shared conditions. Sustained warm behaviour is interpreted as more suggestive of insufficient cooling than unusually cold behaviour. Common-mode cooling problems, multiple simultaneous faults, or unequal operating conditions could violate this assumption.
- **Pressure and missing-data assumptions.** Internal circuit mismatch is useful only when pressure and compressor measurements are available and comparable. Its support is limited to one labelled rich-schema case. Invalid readings and the 10–50 temperature plausibility range are data-quality rules for the supplied telemetry, not learnt proof of a leak. Unsupported cars remain in the required ranking but follow cars with usable evidence; this deterministic fallback must not be read as a healthy diagnosis.
- **Model retention and uncertainty.** The fixed gap-aware candidate was not adopted because it did not improve the prescribed comparison metrics. No split or candidate was selected from the unlabelled test workbook's unknown fault outcome. The ranker returns an ordering and comparable evidence scores, not calibrated fault probabilities; independent labelled cases are needed to establish generalisation.
...
`Optional_Items/ACV/code/outputs/robustness_results.json` records the exact protocol, data cadence/gap counts, schema-specific summaries, feature changes, all 312 paired observation rows and the decision. Current full-workbook rankings were checked against the existing validation report, and each observation was checked against the official rank-decay formula.

## Reproduction and Interface

```bash
python scripts/model.py acv train \
  --data-dir PS3/02_Datasets/ACV \
  --model-out app/backend/artifacts/acv_pipeline.joblib

python scripts/model.py acv predict \
